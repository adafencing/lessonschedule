...
const empty = {
  date: today(),
  start: "18:00",
  // end will be computed from start + duration
  student: "",
  status: "scheduled",
  notes: ""
};

// --- time helpers (local to this file) ---
function timeToMinutes(hhmm = "00:00") {
  const [h, m] = (hhmm || "00:00").split(":").map(Number);
  return (h * 60) + (m || 0);
}
function minutesToTime(total) {
  const t = ((total % (24*60)) + (24*60)) % (24*60); // keep in 0..1439
...
export default function LessonForm({ initial, onSubmit, onCancel }){
  ...
  // When editing, derive duration from existing start/end if available
  useEffect(()=>{ 
  if (initial) {
    setForm(prev => ({ ...empty, ...initial })); // merge defaults + prefill
    if (initial.start && initial.end) {
      const d = Math.max(0, diffMinutes(initial.start, initial.end));
      if (d) setDuration(d);
    }
  } else {
    setForm(empty);
  ...
  // Build duration options (include existing if it isn't 20/30/40)
    ...
    <form className="grid cols-3" onSubmit={submit}>
      <div className="card">
        ...
        <div className="small" style={{ marginTop: 6 }}>
          End time will be set automatically to <strong>{addMinutesToTime(form.start, duration)}</strong>.
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="row" style={{marginTop:12, justifyContent:"flex-end"}}>
          ...
          <button className="btn primary" type="submit">{initial ? "Update" : "Add lesson"}</button>
        </div>
      </div>
    </form>
  );
